...
function parseFilters(list, config = new config_1.default()) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (filterType === 1 /* FilterType.NETWORK */ && config.loadNetworkFilters === true) {
            ...
            const preprocessorToken = (0, preprocessor_1.detectPreprocessor)(line);
            if (preprocessorToken === 1 /* PreprocessorTokens.BEGIF */) {
                if (preprocessorStack.length > 0) {
                    preprocessorStack.push(new preprocessor_1.default({
                        condition: `(${preprocessorStack[preprocessorStack.length - 1].condition})&&(${preprocessor_1.default.getCondition(line)})`,
                    }));
                }
                else {
                    preprocessorStack.push(preprocessor_1.default.parse(line));
                }
            }
...
function getLinesWithFilters(list, config = new config_1.default()) {
    ...
    return new Set(getFilters(list, new config_1.default(Object.assign({}, config, { debug: true }))).filters.map(({ rawLine }) => rawLine));
}
