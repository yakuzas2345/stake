    ...
    updateId(hash) {
        ...
        if (hostnames !== undefined) {
            for (const hostname of hostnames) {
                hash = (hash * utils_1.HASH_INTERNAL_MULT) ^ hostname;
            }
        }
        if (entities !== undefined) {
            for (const entity of entities) {
                hash = (hash * utils_1.HASH_INTERNAL_MULT) ^ entity;
            }
        }
        if (notHostnames !== undefined) {
            for (const notHostname of notHostnames) {
                hash = (hash * utils_1.HASH_INTERNAL_MULT) ^ notHostname;
            }
        }
        if (notEntities !== undefined) {
            for (const notEntity of notEntities) {
                hash = (hash * utils_1.HASH_INTERNAL_MULT) ^ notEntity;
            }
        }
        return hash;
    }
